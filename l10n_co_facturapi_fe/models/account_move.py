...
class AccountMove(models.Model):
    ...
    l10n_co_facturapi_document_id = fields.Many2one(
        "facturapi.document",
        string="DIAN Document",
        compute="_compute_l10n_co_facturapi_document",
        store=True,
    )
    l10n_co_facturapi_state = fields.Selection(
        related="l10n_co_facturapi_document_id.state",
        string="DIAN Status",
    )
    ...
    l10n_co_qr_code = fields.Text(
        string="QR Code",
        readonly=True,
        copy=False,
    )
    l10n_co_xml_signed = fields.Text(
        string="XML Signed",
        readonly=True,
        copy=False,
    )
    l10n_co_application_response = fields.Text(
        string="Application Response",
        readonly=True,
        copy=False,
    )
    l10n_co_sequence_range_id = fields.Many2one(
    ...
    def _compute_l10n_co_facturapi_document(self):
        for move in self:
            ...
            move.l10n_co_facturapi_document_id = doc

    def _l10n_co_facturapi_post(self):
        self.ensure_one()
        if self.l10n_co_facturapi_document_id and self.l10n_co_facturapi_document_id.state != "to_send":
    ...
    def _l10n_co_facturapi_cancel(self):
        self.ensure_one()
        doc = self.l10n_co_facturapi_document_id
    ...
    def _l10n_co_facturapi_check_status(self):
        self.ensure_one()
        doc = self.l10n_co_facturapi_document_id
        if doc:
    ...
    def action_check_dian_status(self):
        for move in self:
            move._l10n_co_facturapi_check_status()

    def action_cancel_dian(self):
        for move in self:
            move._l10n_co_facturapi_cancel()

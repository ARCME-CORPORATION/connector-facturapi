...
class FacturAPIEDiFormat(models.Model):
    ...
    def _get_move_applicability(self, move):
        ...
        if move.move_type not in ("out_invoice", "out_refund", "out_debit"):
            return
        if not move.company_id.l10n_co_fe_enabled:
            return
        return {
            "post": self._l10n_co_facturapi_post_invoices,
            "cancel": self._l10n_co_facturapi_cancel_invoices,
            "edi_content": self._l10n_co_facturapi_edi_content,
    ...
    def _l10n_co_facturapi_post_invoices(self, moves):
        result = {}
        for move in moves:
            try:
    ...
    def _l10n_co_facturapi_cancel_invoices(self, moves):
        result = {}
        for move in moves:
            try:
    ...
    def _l10n_co_facturapi_edi_content(self, move):
        doc = move.l10n_co_facturapi_document_id
...
def _build_party_dict(partner, company=None):
    ...
    tax_level_code = getattr(company, "l10n_co_tax_level_code", "") or "0" if company else "0"
    return {

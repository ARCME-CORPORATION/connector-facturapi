...
class AccountMove(models.Model):
    ...
    connector_ds_document_id = fields.Many2one(
        "facturapi.document",
        string="DIAN DS Document",
        compute="_compute_connector_ds_document",
        store=True,
    )
    connector_ds_state = fields.Selection(
        related="connector_ds_document_id.state",
        string="DIAN DS Status",
    )
    connector_cuds = fields.Char(
        string="CUDS",
        size=96,
        readonly=True,
        copy=False,
    )
    connector_ds_qr_code = fields.Text(
        string="DS QR Code",
        readonly=True,
        copy=False,
    )
    connector_ds_xml_signed = fields.Text(
        string="DS XML Signed",
        readonly=True,
        copy=False,
    )
    connector_ds_application_response = fields.Text(
        string="DS Application Response",
        readonly=True,
        copy=False,
    )
    connector_ds_sequence_range_id = fields.Many2one(
    ...
    def _compute_connector_ds_document(self):
        for move in self:
            ...
            move.connector_ds_document_id = doc

    def _connector_ds_post(self):
        self.ensure_one()
        if self.connector_ds_document_id and self.connector_ds_document_id.state != "to_send":
    ...
    def _connector_ds_cancel(self):
        self.ensure_one()
        doc = self.connector_ds_document_id
    ...
    def _connector_ds_check_status(self):
        self.ensure_one()
        doc = self.connector_ds_document_id
        if doc:
    ...
    def action_check_ds_status(self):
        for move in self:
            move._connector_ds_check_status()

    def action_cancel_ds(self):
        for move in self:
            move._connector_ds_cancel()

...
class AccountEdiFormatDS(models.Model):
    ...
    def _get_move_applicability(self, move):
        ...
        if self.code == "facturapi_ds":
            if move.move_type not in ("out_invoice", "out_refund"):
                return
            if not move.company_id.connector_ds_enabled:
                return
            return {
                "post": self._connector_ds_post_invoices,
                "cancel": self._connector_ds_cancel_invoices,
                "edi_content": self._connector_ds_edi_content,
                "post_batching": lambda move: (move.move_type,),
                "cancel_batching": lambda move: (move.move_type,),
            }
        return super()._get_move_applicability(move)

    def _connector_ds_post_invoices(self, moves):
        result = {}
        for move in moves:
            try:
    ...
    def _connector_ds_cancel_invoices(self, moves):
        result = {}
        for move in moves:
            try:
    ...
    def _connector_ds_edi_content(self, move):
        doc = move.connector_ds_document_id
    ...
    def _build_ds_party_dict(self, partner, company=None, is_supplier=False):
        ...
        if is_supplier:
            ...
            return {
                ...
                "tax_scheme_id": "01",
                "tax_scheme_name": "IVA",
                "tax_level_code": str(getattr(company, "connector_tax_level_code", "0") or "0") if company else "0",
                "fiscal_regime": str(fiscal_regime),
